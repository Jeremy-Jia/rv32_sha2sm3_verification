...
uint32_t sha512_sum0r(uint32_t rs1, uint32_t rs2)
{
...
uint32_t sha512_sum1r(uint32_t rs1, uint32_t rs2)
{
...
#define SHA512R(x0, x1, x2, x3, x4, x5, x6, x7, 		\
	...
	th = (xd ^ (x9 & (xb ^ xd)));						\
	ADD64(xe, xf, xe, xf, tl, th);						\
	tl = sha512_sum1r(x8, x9);							\
	th = sha512_sum1r(x9, x8);							\
	ADD64(xe, xf, xe, xf, tl, th);						\
	ADD64(x6, x7, x6, x7, xe, xf);						\
	tl = sha512_sum0r(x0, x1);							\
	th = sha512_sum0r(x1, x0);							\
	ADD64(xe, xf, xe, xf, tl, th);						\
	tl = (((x0 | x4) & x2) | (x4 & x0));				\
	th = (((x1 | x5) & x3) | (x5 & x1));				\
	ADD64(xe, xf, xe, xf, tl, th);						}
